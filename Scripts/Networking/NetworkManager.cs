...
public partial class NetworkManager : Node
{
	...
	public class PlayerInfo
	{
		...
		public PlayerRole SelectedRole { get; set; } = PlayerRole.None;
	}
	...
	private string SerializePlayers()
	{
		...
		foreach (var p in LobbyPlayers)
			entries.Add($"{p.PeerId}:{EncodeField(p.Name)}:{p.Runs}:{p.IsReady}:{(int)p.SelectedRole}");
	...
	private void DeserializePlayers(string data)
	{
		...
		foreach (var part in data.Split('|'))
		{
			...
			int.TryParse(f.Length > 4 ? f[4] : "0", out int roleInt);
			LobbyPlayers.Add(new PlayerInfo { PeerId = peerId, Name = DecodeField(f[1]), Runs = runs, IsReady = isReady, SelectedRole = (PlayerRole)roleInt });
		}
		EmitSignal(SignalName.LobbyPlayersUpdated);
	}
	...
	public void SelectRole(PlayerRole role)
	{
		...
		if (Multiplayer.IsServer()) UpdatePlayerRoleRPC((int)role);
		else RpcId(ServerPeerId, nameof(UpdatePlayerRoleRPC), (int)role);
	}

	[Rpc(MultiplayerApi.RpcMode.AnyPeer, CallLocal = false)]
	private void UpdatePlayerRoleRPC(int role)
	{
		long senderId = Multiplayer.GetRemoteSenderId();
		if (senderId == 0) senderId = 1;
		...
		PlayerRole selectedRole = (PlayerRole)role;
		if (!IsValidSelectableRole(selectedRole))
			selectedRole = PlayerRole.None;

		if (selectedRole != PlayerRole.None && LobbyPlayers.Exists(p => p.PeerId != senderId && p.SelectedRole == selectedRole))
			selectedRole = PlayerRole.None;

		player.SelectedRole = selectedRole;
		Rpc(nameof(SyncLobbyPlayers), SerializePlayers());
	}

	private static bool IsValidSelectableRole(PlayerRole role)
	{
		return role is PlayerRole.None
			or PlayerRole.Loud
			or PlayerRole.Static
			or PlayerRole.Mute
			or PlayerRole.Archivist
			or PlayerRole.Witness;
	}
