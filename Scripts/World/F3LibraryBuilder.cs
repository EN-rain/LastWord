...
/// <summary>
/// Procedural art pass for F3_Library. Large book-lined chamber with central Silence Room.
/// Uses KayKit Dungeon Remastered (CC0).
/// </summary>
public partial class F3LibraryBuilder : Node3D
{
    ...
    private PackedScene Load(string path)
    {
        ...
        if (loaded == null)
        {
            GD.PushWarning($"F3LibraryBuilder: could not load {path}");
            return null;
        }
        _cache[path] = loaded;
        return loaded;
    ...
    public override void _Ready()
    {
        ...
        GD.Print("F3LibraryBuilder: starting art pass...");
        BuildFloor();
        BuildWalls();
        BuildProps();
        BuildLighting();
        GD.Print($"F3LibraryBuilder: art pass complete. {GetChildCount()} visual nodes generated.");
    }
    ...
    private void BuildFloor()
    {
        for (int x = -2; x < 2; x++)
            for (int z = -2; z < 2; z++)
                Place(FloorWoodDark, new Vector3(x * 5f + 2.5f, 0f, z * 5f + 2.5f), Vector3.Zero, new Vector3(5f, 1f, 5f));
    ...
    private void BuildWalls()
    {
        ...
        for (int x = -2; x < 2; x++)
        {
            ...
            Place(Wall, new Vector3(xPos, 0f, 10f), new Vector3(0f, 180f, 0f), new Vector3(5f, 3.5f, 1f));
        }
    ...
    {
        // Bookshelves along walls represented by decorated pillars / crates stacks
        for (int z = -8; z <= 8; z += 4)
        {
            Place(CratesStacked, new Vector3(-9f, 0f, z), new Vector3(0f, 90f, 0f), new Vector3(1f, 1.2f, 0.5f));
            Place(CratesStacked, new Vector3(9f, 0f, z), new Vector3(0f, -90f, 0f), new Vector3(1f, 1.2f, 0.5f));
        }
        for (int x = -8; x <= 8; x += 4)
        {
            Place(CratesStacked, new Vector3(x, 0f, -9f), Vector3.Zero, new Vector3(1f, 1.2f, 0.5f));
            Place(CratesStacked, new Vector3(x, 0f, 9f), new Vector3(0f, 180f, 0f), new Vector3(1f, 1.2f, 0.5f));
        }

        // Central silence room pillars
        Place(PillarDecorated, new Vector3(-2.5f, 0f, -2.5f), Vector3.Zero, new Vector3(1f, 3.5f, 1f));
        Place(PillarDecorated, new Vector3(2.5f, 0f, -2.5f), Vector3.Zero, new Vector3(1f, 3.5f, 1f));
        Place(PillarDecorated, new Vector3(-2.5f, 0f, 2.5f), Vector3.Zero, new Vector3(1f, 3.5f, 1f));
        Place(PillarDecorated, new Vector3(2.5f, 0f, 2.5f), Vector3.Zero, new Vector3(1f, 3.5f, 1f));

        Place(Chest, new Vector3(0f, 0f, -7f), Vector3.Zero, Vector3.One * 1.2f);
        Place(BarrelLarge, new Vector3(7f, 0f, 7f), Vector3.Zero, Vector3.One * 1.2f);
    }

    private void BuildLighting()
    {
        AddPointLight(new Vector3(-6f, 2.5f, -6f), new Color(1f, 0.6f, 0.3f), 5f, 1f);
        AddPointLight(new Vector3(6f, 2.5f, -6f), new Color(1f, 0.6f, 0.3f), 5f, 1f);
        AddPointLight(new Vector3(-6f, 2.5f, 6f), new Color(1f, 0.6f, 0.3f), 5f, 1f);
        AddPointLight(new Vector3(6f, 2.5f, 6f), new Color(1f, 0.6f, 0.3f), 5f, 1f);
    }
